...
### Проверка подключения домена leo-japan.ru

1. **GitHub:** **Settings** → **Pages** → в блоке **Custom domain** должно быть указано **leo-japan.ru**, рядом — зелёная галочка «DNS check successful».
2. **Браузер:** откройте **https://leo-japan.ru** (лучше в режиме инкогнито). Должна открыться главная страница лендинга, без редиректа на utromaya-code.github.io.
3. **SSL:** адресная строка должна показывать замок (HTTPS). Если домен только что добавлен, сертификат может появиться в течение часа.
4. **Если не открывается:** проверьте DNS у регистратора (записи A и CNAME из шага 2). Можно проверить через [dnschecker.org](https://dnschecker.org) для leo-japan.ru.

---

## Ошибка «Sorry, something went wrong» на странице Pages

Если в **Settings → Pages** в блоке **Build and deployment** видно «Sorry, something went wrong» под **Select branch** и **Select folder** — это известный сбой интерфейса GitHub.

**Почему так:** источник публикации у вас — **GitHub Actions**. Ветка и папка нужны только для старого варианта «Deploy from a branch». Страница настроек всё равно подгружает эти поля и при обращении к API может получить ошибку — тогда и показывается это сообщение.

**Что важно:** если выше написано, что сайт **live at https://leo-japan.ru/** и деплой прошёл — публикация работает, на работу сайта эта надпись не влияет.

**Что можно сделать (по желанию):** обновить страницу (F5), сменить браузер или подождать — иногда сообщение пропадает. Менять источник на «Deploy from a branch» не нужно: для этого репозитория используется только GitHub Actions.
